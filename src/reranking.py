...
class LLMReranker:
    def __init__(self, provider: str = "dashscope", model: Optional[str] = None):
    ...
    def get_rank_for_single_block(self, query, retrieved_document):
        ...
        if self.provider == "openai":
            model = self.model if self.model else "gpt-4o-mini-2024-07-18"
            system_prompt = self.system_prompt_rerank_single_block + "\n\nYour response MUST be in JSON format."
            try:
                ...
                print(f"Reranker OpenAI parse failed: {e}. Falling back to manual.")
            ...
            if not rsp or not hasattr(rsp, 'status_code'):
                raise RuntimeError(f"DashScope返回异常: {rsp}")
    ...
    def get_rank_for_multiple_blocks(self, query, retrieved_documents):
        ...
        if self.provider == "openai":
            model = self.model if self.model else "gpt-4o-mini-2024-07-18"
            system_prompt = self.system_prompt_rerank_multiple_blocks + "\n\nYour response MUST be in JSON format."
            try:
                completion = self.llm.beta.chat.completions.parse(
                    model=model,
                    temperature=0,
                ...
                print(f"Reranker OpenAI parse failed (multiple): {e}. Falling back to manual.")
                completion = self.llm.chat.completions.create(
                    model=model,
                    temperature=0,
                    messages=[
